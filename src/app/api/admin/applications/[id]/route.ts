...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const status = body?.status !== undefined ? String(body.status).trim() : '';
    if (!status) return NextResponse.json({ ok: false, error: 'Status inválido' } satisfies ResponsePayload, { status: 400 });
 
    const now = new Date().toISOString();
    const { error: updErr } = await (admin as any).from('applications').update({ status, updated_at: now }).eq('id', applicationId);
