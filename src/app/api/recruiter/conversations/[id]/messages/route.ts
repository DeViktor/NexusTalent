...
type ResponsePayload =
  | { ok: true; message: { id: string; sender: 'recruiter'; text: string; timestamp: string } }
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (text.length > 5000) return NextResponse.json({ ok: false, error: 'Mensagem muito longa' } satisfies ResponsePayload, { status: 400 });
    ...
    return NextResponse.json(
      {
        ok: true,
        message: { id: String(created.id), sender: 'recruiter', text: String(created.body), timestamp: String(created.created_at) },
