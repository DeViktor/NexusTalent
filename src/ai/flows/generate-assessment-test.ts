...
type Runtime = {
  generateAssessmentTestFlow: (input: GenerateAssessmentTestInput) => Promise<GenerateAssessmentTestOutput>;
};
...
6.  **IMPORTANT:** Ensure all multiple-choice questions have exactly 4 plausible options and specify the correct answer by setting 'correctAnswerIndex'.`,
  ...
  const runtime = { generateAssessmentTestFlow };
  runtimeCache.set(cacheKey, runtime);
  return runtime;
}

export async function generateAssessmentTest(input: GenerateAssessmentTestInput, apiKey?: string | null): Promise<GenerateAssessmentTestOutput> {
  return getRuntime(apiKey).generateAssessmentTestFlow(input);
}
